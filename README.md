...
2. **Build a Grader** - An `output.csv` file has been included in the folder with [titanic dataset](./titanic). It has the expected predictions corresponding to [test data](./titanic/test.csv) and your task is to write a Python fucntion which takes as input a csv filename, compares to the expected output and prints the percentage of accuracy achieved.
